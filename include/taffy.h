...
namespace Taffy {
    ...
    enum class FeatureFlags : uint64_t {
        ...
        EmbeddedShaders = 1 << 7,   // SPIR-V shaders (v0.2+)
        MeshShaders = 1 << 8,       // Mesh shader support (v0.3+)
        SPIRVCross = 1 << 9,        // Universal shader transpilation (v0.3+)
        ProceduralGeometry = 1 << 10, // Geometry generated by shaders
    ...
    /**
     * Enhanced shader chunk - SPIR-V shaders with mesh shader support (v0.3+)
     */
    struct ShaderChunk {
        ...
        enum class ShaderStage : uint32_t {
            ...
            TaskShader = 0,      // Task shader (pre-mesh processing) 
            MeshShader = 1,      // Mesh shader (replaces vertex shader)
            Fragment = 2,        // Fragment shader
            Compute = 3,         // Compute shader

            // Legacy pipeline support
            Vertex = 10,         // Traditional vertex shader
            Geometry = 11,       // Geometry shader
            TessControl = 12,    // Tessellation control
            TessEvaluation = 13  // Tessellation evaluation
        };

        enum class TargetAPI : uint32_t {
            SPIRV = 0,          // Raw SPIR-V (Vulkan native)
            GLSL = 1,           // OpenGL/Vulkan GLSL
            HLSL = 2,           // DirectX HLSL
            MSL = 3,            // Metal Shading Language
            WebGL = 4           // WebGL-compatible GLSL
        };

        struct Shader {
            char name[64];           // "animated_mesh_shader"
            ShaderStage stage;       // Task, Mesh, Fragment, etc.
            uint32_t spirv_size;     // Size of SPIR-V bytecode
            uint32_t entry_point_offset; // Offset to entry point name
            char entry_point[32];    // "main" usually

            // Mesh shader specific parameters
            uint32_t max_vertices;   // Max vertices output by mesh shader
            uint32_t max_primitives; // Max primitives output by mesh shader
            uint32_t workgroup_size[3]; // Workgroup dimensions [x,y,z]

            uint32_t flags;          // Shader-specific flags
            uint32_t _reserved[2];

            // SPIR-V bytecode follows this structure
        };

        // Array of shaders follows
    };
    ...
    public:
        ...
        std::optional<ShaderChunk> get_shaders() const {
            auto data = get_chunk_data(ChunkType::SHDR);
            if (!data || data->size() < sizeof(ShaderChunk)) {
                return std::nullopt;
            }

            ShaderChunk shaders;
            std::memcpy(&shaders, data->data(), sizeof(ShaderChunk));
            return shaders;
        }
    ...
    inline const char* chunk_type_to_string(ChunkType type) {
        ...
        case ChunkType::SHDR: return "Mesh Shaders";  // Updated for mesh shader support
    ...
    inline bool engine_supports_features(FeatureFlags required_features) {
        // For v0.3, we support advanced shader features
        FeatureFlags supported_features =
            ...
            FeatureFlags::ProceduralGeometry;
