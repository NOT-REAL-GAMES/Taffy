...
bool createWireframeShaderOverlay(const std::string& output_path) {
    std::cout << "📐 Creating wireframe shader overlay (hash-based)..." << std::endl;
    
    using namespace Taffy;
    
    // Compile wireframe shader
    TaffyAssetCompiler compiler;
    std::string wireframe_glsl = R"(
#version 460
...
void main() {
    ...
    for (int i = 0; i < 6; ++i) {
        fragColor[i] = vec4(1.0, 1.0, 1.0, 1.0); // White wireframe
    }
    ...
    auto wireframe_spirv = compiler.compileGLSLToSpirv(wireframe_glsl, shaderc_mesh_shader, "wireframe_mesh_shader");
    if (wireframe_spirv.empty()) {
        std::cerr << "❌ Failed to compile wireframe shader!" << std::endl;
        return false;
    }
    
    Overlay overlay;
    overlay.add_target_asset("assets/tri.taf", "^1.0.0");
    
    // Replace triangle mesh shader with wireframe mesh shader
    uint64_t target_hash = ShaderHashes::TRIANGLE_MESH;  // Original shader
    uint64_t replacement_hash = HashRegistry::register_and_hash("wireframe_mesh_shader");  // New shader
    
    overlay.add_shader_replacement(target_hash, replacement_hash, wireframe_spirv);
    
    std::filesystem::create_directories(std::filesystem::path(output_path).parent_path());
    
    if (overlay.save_to_file(output_path)) {
        std::cout << "✅ Wireframe overlay created!" << std::endl;
        std::cout << "   🎯 Replaces triangle mesh shader with wireframe" << std::endl;
        std::cout << "   📐 Target hash: 0x" << std::hex << target_hash << std::dec << std::endl;
        std::cout << "   🔄 Replacement hash: 0x" << std::hex << replacement_hash << std::dec << std::endl;
        return true;
    }
